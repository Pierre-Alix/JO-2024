...
<body>
    ...
    <main>
        ...
        try {
            ...
            $query = "SELECT * FROM UTILISATEUR ORDER BY prenom_utilisateur";
            $statement = $connexion->prepare($query);
            $statement->execute();

            // Vérifier s'il y a des résultats
            if ($statement->rowCount() > 0) {
                echo "<table><tr><th>Utilisateurs</th><th>Modifier</th><th>Supprimer</th></tr>";

                // Afficher les données dans un tableau
                while ($row = $statement->fetch(PDO::FETCH_ASSOC)) {
                    echo "<tr>";
                    // Assainir les données avant de les afficher
                    echo "<td>" . htmlspecialchars($row['prenom_utilisateur']) . " " . htmlspecialchars($row['nom_utilisateur']) . "</td>";
...
// (fonctionne à condition d’avoir activé l’option en local)
    ...
    <script>
        ...
        function openModifyUserForm(id_user) {
            // Ajoutez ici le code pour afficher un formulaire stylisé pour modifier un utilisateur
            // alert(id_user);
            window.location.href = 'modify-user.php?id_utilisateur=' + id_user;
            
        }

        function deleteUserConfirmation(id_user) {
            ...
            if (confirm("Êtes-vous sûr de vouloir supprimer ce utilisateur?")) {
                // Ajoutez ici le code pour la suppression de l'utilisteur
                // alert(id_user);
                window.location.href = 'delete-user.php?id_utilisateur=' + id_user;
            }
        }
    </script>
</body>
